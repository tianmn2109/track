...
(function ($) {
	...
	*/
    var parseNode = function (source) {
        ...
        // highlighted?
        var highlighted = source.charAt(0) === "(" ? 1 : 0;
        if (highlighted == 1) { source = trim(source.substring(1)); }
        if (source.charAt(0) == "(") { highlighted = 2; }
        if (highlighted == 2) { source = trim(source.substring(1)); }
	...
	*/
    var structureForNode = function (node) {
        ...
        if (node["highlighted"] == 2) {
            res.addClass(cssclass("node-delay"));
        }
        if (!is_dummy) {
            if (node["title"] != "") {
	...
	*/
    var drawArrow = function (arrow, target, outer_wrapper, pos) {
        // TODO actually draw ;)		
        var color = CONFIG["arrow"][arrow["highlighted"] == 0 ? "color_normal" : (arrow["highlighted"] == 1 ? "color_highlighted" : "color_delay")];
